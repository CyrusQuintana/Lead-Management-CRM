      ...
      <SectionMain>
        ...
        <CardBox>
          <div className={'mb-4'}>
            ...
            <p>{interactions?.lead?.first_name ?? 'No data'}</p>
          </div>

          <div className={'mb-4'}>
            <p className={'block font-bold mb-2'}>User</p>

            <p>{interactions?.user?.firstName ?? 'No data'}</p>
          </div>
            ...
            {interactions.interaction_date ? (
              <DatePicker
                ...
                selected={
                  interactions.interaction_date
                    ? new Date(
                        dayjs(interactions.interaction_date).format(
                          'YYYY-MM-DD hh:mm',
                        ),
                      )
                    : null
          ...
          {hasPermission(currentUser, 'READ_ORGANIZATIONS') && (
            <div className={'mb-4'}>
              ...
              <p>{interactions?.organization?.name ?? 'No data'}</p>
            </div>
          )}
